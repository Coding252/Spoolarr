...
public class NfcTagControllerTests
{
    ...
    public NfcTagControllerTests()
    {
        ...
        _sut = new NfcTagController(_nfcTagService, _nfcScanService, _hubContext, NullLogger<NfcTagController>.Instance);
    }
    ...
    [Fact]
    public async Task ScanTag_WhenTagUidIsEmpty_ReturnsBadRequest()
    {
        var result = await _sut.ScanTag(new ScanRequest(string.Empty));

        Assert.IsType<BadRequestObjectResult>(result);
    }

    [Fact]
    public async Task ScanTag_WhenTagUidIsWhitespace_ReturnsBadRequest()
    {
        var result = await _sut.ScanTag(new ScanRequest("   "));
    ...
    [Fact]
    public async Task ScanTag_WhenTagUidIsValid_PushesResultToHub()
    {
        var scanResult = new NfcScanResult("unknown", "04:AA:BB:CC", null, "Tag not registered");
        _nfcScanService.ProcessScanAsync("04:AA:BB:CC").Returns(scanResult);

        await _sut.ScanTag(new ScanRequest("04:AA:BB:CC"));
