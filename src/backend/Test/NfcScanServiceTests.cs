...
public class NfcScanServiceTests
{
    ...
    public NfcScanServiceTests() => _sut = new NfcScanService(_nfcRepo, _spoolService);
    ...
    [Fact]
    public async Task ProcessScanAsync_WhenTagFound_ReturnsActivated()
    {
        ...
        _spoolService.ActivateAsync(spoolId).Returns(spoolResponse);
    ...
    [Fact]
    public async Task ProcessScanAsync_WhenTagFound_CallsActivateWithCorrectId()
    {
        var spoolId = Guid.NewGuid();
        var tag = new NfcTag { TagUid = "04:AA:BB:CC", SpoolId = spoolId };

        _nfcRepo.GetByTagUidAsync("04:AA:BB:CC").Returns(tag);
        _spoolService.ActivateAsync(spoolId).Returns(BuildSpoolResponse(spoolId));

        await _sut.ProcessScanAsync("04:AA:BB:CC");

        await _spoolService.Received(1).ActivateAsync(spoolId);
    }

    private static SpoolResponse BuildSpoolResponse(Guid id) => new(
        id, "Bambu", "PLA", "White", "#FFFFFF",
        1000, 800, 200, 1.75f, 100, true, false,
